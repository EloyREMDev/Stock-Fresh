...
import { supplySchema } from './supply.schemas.js';

const supplyIdSchema = z
  .string()
  .transform((val) => Number(val))
  .refine((val) => !isNaN(val), 'El id del insumo debe ser un numero.');

export const createSupplyRouteSchema = {
  params: z.object({}),
  body: supplySchema.omit({ id: true }),
  queries: z.object({}),
};

export const deleteSupplyRouteSchema = {
  params: z.object({ id: supplyIdSchema }),
  body: z.object({}),
  queries: z.object({}),
};
//
export const updateSupplyRouteSchema = {
  params: z.object({ id: supplyIdSchema }),
  body: supplySchema.omit({ id: true }),
  queries: z.object({}),
};
